...
POST /tasks/summarise               ← Summarisation
        │
        ▼
POST /tasks/sunflower_simple        ← Translation (Sunflower LLM)
        │
        ▼
...
├── backend/
│   ├── sunbird_client.py       # Thin wrapper around all Sunbird API endpoints
│   └── pipeline.py             # Orchestrates the full STT → summarise → translate → TTS flow
├── exercises/
│   └── basics.py               # Part 1: collatz and distinct_numbers implementations
├── tests/
│   └── test_basics.py          # Pytest test suite for Part 1
...
Open `.env` and replace `your_token_here` with your actual Sunbird AI API token:

```
SUNBIRD_API_TOKEN=eyJ...your_actual_token...
```
...
|---|---|
| `SUNBIRD_API_TOKEN` | Your Sunbird AI API bearer token. Required for all API calls. Obtain from [https://api.sunbird.ai/](https://api.sunbird.ai/). |
...
> **[https://huggingface.co/spaces/<your-username>/sunbird-pipeline](https://huggingface.co/spaces/<your-username>/sunbird-pipeline)**
>
...
| Name | Value |
|---|---|
| `SUNBIRD_API_TOKEN` | your actual token |
